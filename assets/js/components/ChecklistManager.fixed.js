...
export class ChecklistManager {
    ...
    async render() {
        if (!this.container || !window.planData) {
            this.debugLog('[ChecklistManager] Missing container or planData');
            return;
        }

        const dayData = window.planData[`Dan ${this.currentDay}`];
        if (!dayData || !dayData.checklist) {
            this.debugLog(`[ChecklistManager] No checklist data for day ${this.currentDay}`);
            return;
        }
